...
SAMPLE_RATE = 8000
BUFFER_HOURS = 12
AUDIO_SERVER_ADDRESS = ('0.0.0.0', 6000)
...
def process_audio(shared_audio, shared_time, shared_pos, lock):
    ...
    p = pyaudio.PyAudio()
    stream = p.open(format=AUDIO_FORMAT, channels=1, rate=SAMPLE_RATE, input=True, input_device_index=0, frames_per_buffer=CHUNK_SIZE)
...
def process_data(shared_audio, shared_time, shared_pos, lock, mqtt_client):
    print('asdf')
    starttime=time.time()
    time.sleep(20)
    while True:
        ...
        if len(crying_blocks) == 0:
            mqtt_client.publish("home/baby/","off")
            ...
            if time_current - crying_blocks[-1]['stop'] < parameters['min_quiet_time']:
                ...
                mqtt_client.publish("home/baby/","on")
            else:
...
def init_server():
    ...
    mqtt_client.connect(MQTT_BROKER)#connect
    mqtt_client.publish("home/baby/","off")
    ...
    p2 = mp.Process(target=process_data, args=(shared_audio, shared_time, shared_pos, lock, mqtt_client))
    p1.start()
    p2.start()
